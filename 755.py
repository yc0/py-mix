...
class Solution:
    def pourWater(self, heights, V, K):
        ...
        for _ in range(V):
            for d in (-1,1):
                cur = best = K
                while 0 <= cur+d < n and H[cur+d] <= H[cur]:
                    if H[cur+d] < H[cur]:
                        best = cur+d
                    cur += d
                if best != K:
...
if __name__ == '__main__':
    ...
    assert s.pourWater([2,1,1,2,1,2,2],4,3)==[2, 2, 2, 3, 2, 2, 2]
    assert s.pourWater([1,2,3,4],2,2) == [2,3,3,4]
    assert s.pourWater([3,1,3],5,1) == [4,4,4]
    
    try :
        assert s.pourWater([1,2,3,4,3,2,1,2,3,4,3,2,1],10,2)==[4,4,4,4,3,3,3,3,3,4,3,2,1]
        print('passed all')
